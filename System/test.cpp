...
TEST_CASE("Phillip se testing")
{
    ...
    shop.setState(new DemolishedState());
...
TEST_CASE("Iterator class functionality") {
    ...
    /* SUBCASE("Testing getHouse") {
        auto* factory = new ResidentialBuildingFactory("House", 0, 0);
        auto* house = factory->createBuilding();
    ...
    SUBCASE("Testing getJob") {
        auto* factory = new CommercialBuildingFactory("Shop", 1, 1);
        auto* shop = factory->createBuilding();
